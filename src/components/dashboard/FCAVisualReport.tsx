    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
            ...
            if (proposedCR > 120) {
              if (currentCR > 120) {
                ...
                return [
                  ...
                  new Paragraph({
                    text: `This proposition is the result of:\n- 50% of ${macroEffectValue.toFixed(2)}% for macroeconomic changes = ${macroContribution.toFixed(2)}%\n- LM's assessment of the SOW of "${performanceRating}" with a compa-ratio of ${Math.ceil(currentCR)}% meaning a ${performanceContribution.toFixed(2)}% increase`,
                ...
                return [
                  ...
                  new Paragraph({
                    text: `This proposition is the result of:\n- 50% of ${macroEffectValue.toFixed(2)}% for macroeconomic changes = ${macroContribution.toFixed(2)}%\n- LM's assessment of the SOW of "${performanceRating}" with a compa-ratio of 120% meaning a ${performanceContribution.toFixed(2)}% increase`,
                    spacing: { after: selectedAnalysis.rationale ? 100 : 200 },
                  }),
                ];
              }
              ...
              return [
                ...
                new Paragraph({
                  text: `This proposition is the result of:\n- 50% of ${macroEffectValue.toFixed(2)}% for macroeconomic changes = ${macroContribution.toFixed(2)}%\n- LM's assessment of the SOW of "${performanceRating}" with a compa-ratio of ${Math.ceil(proposedCR)}% meaning a ${performanceContribution.toFixed(2)}% increase`,
                  spacing: { after: selectedAnalysis.rationale ? 100 : 200 },
                }),
              ];
            }
          ...
          // Our Initial Offer
          new Paragraph({
            children: [new TextRun({ text: "Our Initial Offer", bold: true, size: 28 })],
            spacing: { before: 200, after: 100 },
          }),
          new Paragraph({
            children: [new TextRun({ text: "To begin discussions", italics: true })],
            spacing: { after: 100 },
          }),
          ...(() => {
            const proposedCR = (selectedAnalysis.proposed_salary / midpoint) * 100;
  ...
  const handleExportToExcel = () => {
    ...
    getHumanForceData().then((humanforceData) => {
      const exportData = [
        ...
        { Section: "Performance Contribution", Details: `${((((selectedAnalysis.proposed_salary - selectedAnalysis.current_salary) / selectedAnalysis.current_salary) * 100) - (Math.abs(parseFloat(macroEffect || "0")) * 0.5)).toFixed(2)}%` },
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell className="whitespace-pre-line">
                  ...
                  {(() => {
                    ...
                    const macroEffect = feeApprovalData?.document_content?.formData?.macroeconomic_effect;
                    const macroEffectValue = macroEffect ? Math.abs(parseFloat(macroEffect)) : 0;
                    const macroContribution = macroEffectValue * 0.5; // 50% of macroeconomic effect
                    const performanceContribution = totalIncrease - macroContribution;
                    const performanceRating = selectedAnalysis.performance_rating || feeApprovalData?.document_content?.formData?.performance_rating || "Meets Expectations";
                    
                    if (proposedCR > 120) {
                      if (selectedAnalysis.compa_ratio_current > 120) {
                        const lumpSum = selectedAnalysis.proposed_salary - selectedAnalysis.current_salary;
                        return `In view of the above and considering the data, our guidelines and the staff's experience, P&C proposes to continue to pay ${Math.ceil(selectedAnalysis.current_salary).toLocaleString()} ${selectedAnalysis.currency} per year which equals to a Compa-ratio of ${Math.ceil(selectedAnalysis.compa_ratio_current)}% and have one time lump sum of ${Math.ceil(lumpSum).toLocaleString()} ${selectedAnalysis.currency} which is a ${totalIncrease.toFixed(2)}% payment. This is within the budgeted ${selectedAnalysis.contract_type === "consultancy" ? "fee" : "salary"} for this role.\n\nThis proposition is the result of:\n- 50% of ${macroEffectValue.toFixed(2)}% for macroeconomic changes = ${macroContribution.toFixed(2)}%\n- LM's assessment of the SOW of "${performanceRating}" with a compa-ratio of ${Math.ceil(selectedAnalysis.compa_ratio_current)}% meaning a ${performanceContribution.toFixed(2)}% increase`;
                      } else {
                        const baseSalaryAt120 = midpoint * 1.20;
                        const lumpSum = selectedAnalysis.proposed_salary - baseSalaryAt120;
                        return `In view of the above and considering the data, our guidelines and the staff's experience, P&C proposes to pay ${Math.ceil(baseSalaryAt120).toLocaleString()} ${selectedAnalysis.currency} per year which equals to a Compa-ratio of 120% and have one time lump sum of ${Math.ceil(lumpSum).toLocaleString()} ${selectedAnalysis.currency} which is a ${totalIncrease.toFixed(2)}% payment. This is within the budgeted ${selectedAnalysis.contract_type === "consultancy" ? "fee" : "salary"} for this role.\n\nThis proposition is the result of:\n- 50% of ${macroEffectValue.toFixed(2)}% for macroeconomic changes = ${macroContribution.toFixed(2)}%\n- LM's assessment of the SOW of "${performanceRating}" with a compa-ratio of 120% meaning a ${performanceContribution.toFixed(2)}% increase`;
              ...
              <TableRow className="bg-primary/5 border-t-2 border-primary">
                <TableCell className="font-bold text-lg">
                  <div>Our Initial Offer</div>
                  <div className="text-xs text-muted-foreground font-normal mt-1">(to begin discussions)</div>
                </TableCell>
                <TableCell className="text-center">
                  <Check className="h-4 w-4 mx-auto text-primary" />
                </TableCell>
                <TableCell className="whitespace-pre-line font-semibold">
                  {(() => {
                    const proposedCR = (selectedAnalysis.proposed_salary / midpoint) * 100;
                    const currentCR = selectedAnalysis.compa_ratio_current;
                    const totalIncrease = ((selectedAnalysis.proposed_salary - selectedAnalysis.current_salary) / selectedAnalysis.current_salary) * 100;
